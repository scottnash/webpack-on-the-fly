window.modules["5"] = [function(require,module,exports){(function (process){
'use strict';

var getJSONP = require(640),
    _defaults = require(894); // global
...
function authenticate(payload) {
  ...
  payload.headers.Authorization = 'Token ' + window.process.env.CLAY_ACCESS_KEY;
...
module.exports.get = function (url, opts) {
  ...
  return fetch(url, conf).then(checkStatus).then(function (res) {
    return res.json();
  });
};
/**
...
module.exports.put = function (url, data, isAuthenticated) {
  ...
  return fetch(url, payload).then(checkStatus).then(function (res) {
    return res.json();
  });
};
/**
...
/**
 * within network use crummashie to purge fastly cache
 * @param  {string}  url
 * @return {Promise}
 */


module.exports.crummashiePurge = function (url) {
  var init = {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    },
    body: "url=".concat(encodeURIComponent(url))
  };
  return fetch('http://crummashie.prd.nymetro.com/invalidate/', init).then(checkStatus);
};

}).call(this,require(22))}, {"22":22,"182":182,"640":640,"894":894}];
