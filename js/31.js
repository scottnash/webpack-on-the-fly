...
var _reduce = require(124),
    ...
    _clone = require(58),
    _remove = require(216),
    Fingerprint2 = require(215),
    cookie = require(28);

var localStorage = window.localStorage,
    uriAttr = 'data-uri',
    visitCountKey = 'visitServiceCount',
    // set in the-factory
previousActionTimestampKey = 'visitServicePreviousTimestamp',
    // set in the-factory
firstVisitTimestampKey = 'visitServiceFirstVisitTimestamp',
    visitStartTimestampKey = 'visitServiceVisitStartTimestamp',
    initialReferrerKey = 'visitServiceInitialRefferer',
    // new to clay
clientIdCookieKey = 'nyma',
    // set on all non-clay pages
visitDatesKey = 'visitDates',
    maxVisitDuration = 1000 * 60 * 60 * 24,
    // a visit can not last more than 24 hours
sessionTTL = 1000 * 60 * 30,
    // keep session open for 30 minutes after last event
visitDatesLimit = 30,
    // number of days over which to track dates of visits
publicState = {},
  ...
  clientId: {
    ...
    set: function set(val) {
      if (val) {
        this._clientId = val;
        cookie.set(clientIdCookieKey, val, {
          expires: 365 * 5
        }); // cookie expires after 5 years
...
function createClientId(currentTimestamp) {
  new Fingerprint2({
    // Fingerprint2 npm module added with gulp
    excludeJsFonts: true,
...
function getQueryParamsObject(keys) {
  ...
  return _reduce(_get(window, 'location.search', '').substr(1).split('&'), function (obj, str) {
    var arr = str.split('='),
        key = arr[0],
        val = arr[1];
...
function setPublicState(clientId, currentTimestamp) {
  ...
  }, getBrowserInfo(), getUtmParams() // could do server-side
  );
...
function executeOnceReadyQueue() {
  onceReadyQueue.forEach(function (callback) {
    return callback(_clone(publicState));
  });
  onceReadyQueue = [];
}
/**
...
function updateVisitDates() {
  var visitDatesStr = localStorage.getItem(visitDatesKey) || '',
      ...
      visitDatesLimitMilliseconds = visitDatesLimit * 24 * 60 * 60 * 1000,
      // visitDatesLimit in milliseconds
  thresholdTimeSinceLastVisit = 24 * 60 * 60 * 1000,
      // 24 hours in milliseconds
  currentTime = Date.now(); // remove dates older than threshold
...
}, {"28":28,"32":32,"33":33,"57":57,"58":58,"124":124,"215":215,"216":216}];
