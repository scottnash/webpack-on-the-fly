window.modules["state-table-2018.client"] = [function(require,module,exports){'use strict';

var _throttle = require(23),
    dom = require(1),
    $gtm = require(41);

DS.controller('state-table-2018', [, function () {
  function Constructor(el) {
    this.el = el;
    this.state = dom.findAll(this.el, '.state-abbreviation');
    this.columnFive = dom.find(this.el, '.col-five');
    this.columnSix = dom.find(this.el, '.col-six');
    this.colSpan = dom.find(this.el, '.col-span');
    this.numSeats = dom.findAll(this.el, '.num-seats');
    this.eventHandlers();
    this.init();
  }
  ...
  Constructor.prototype = {
    ...
    init: function init() {
      ...
      $gtm.reportCustomEvent({
        ...
        label: 'state legislature 2018 midterms',
        action: 'page view'
      });
    },
    ...
    mobileTable: function mobileTable() {
      ...
      if (this.el.classList.contains('mobile')) {
        ...
        this.columnSix.classList.add('hidden');
        this.colSpan.setAttribute('colspan', '4'); // hide the content in the header row

        for (i = 0; i < this.numSeats.length; i++) {
          this.numSeats[i].classList.add('hidden');
        }
      } else {
        this.columnFive.classList.remove('hidden');
        this.columnSix.classList.remove('hidden');
        this.colSpan.setAttribute('colspan', '6'); // show the content in the header row

        for (i = 0; i < this.numSeats.length; i++) {
          this.numSeats[i].classList.remove('hidden');
        }
      }
    },
